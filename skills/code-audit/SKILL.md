...
each rule below has an id so a repo rules file can override one by name. when a rule conflicts with the file's established house style, report the issue, say it loses to house style, and do not fix it
...
- CO-6 prefer no comments or docstrings. allow one short comment when it says why or one short docstring when it adds needed facts
- CO-7 braces for each control flow body in a language with braces. no braceless one-line control flow
- CO-8 write config files bare: no comments in ci yaml or toml, context goes in the commit message
...
- CS-1 xml docs only when a public api needs them. a plain `summary` with no refs is enough. flag xml docs that only repeat the name or type
...
- TE-2 make each test name say what it proves
